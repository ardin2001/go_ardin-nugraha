...
func TestCreateJWTToken(t *testing.T) {
	token, err := JWT.CreateJWTToken(1, "eko_khanedy@gmail.com")
	assert.Nil(t, err)
...
func TestCreateJWTTokenMock_Success(t *testing.T) {
	...
	token, err := JWTMock.CreateJWTToken(1, "eko_khanedy@gmail.com")
	assert.Nil(t, err)
	assert.NotNil(t, token)
}
...
func TestCreateJWTTokenMock_Failure(t *testing.T) {
	...
	token, err := JWTMock.CreateJWTToken(1, "eko_tok@gmail.com")
	assert.NotNil(t, err)
